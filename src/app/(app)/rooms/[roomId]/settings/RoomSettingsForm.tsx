...
export function RoomSettingsForm({ roomId, settings }: Props) {
  ...
  async function handleModelSave(updates: Parameters<typeof updateRoomSettings>[1]) {
    return updateRoomSettings(roomId, updates)
  }

  return (
    <div className="space-y-8">
      <section><DefaultModeSelector initialMode={settings.mode} onSave={handleModeSave} /></section>
      <section><AgentCountSelector initial={activeAgentCount} onSave={handleAgentCountSave} /></section>
      <section>
        <ModelSelector
          roomId={roomId}
          initial={{
      ...
      <section><AutoRunToggleClient roomId={roomId} initial={settings.auto_run_on_user_message} /></section>
    </div>
  )
}
