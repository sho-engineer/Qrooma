...
interface Props { message: Message; locale: Locale }
    ...
    return (
      <div className="flex justify-end px-4 py-1">
        <div className="max-w-[70%]">
          <div className="bg-blue-600 text-white rounded-2xl rounded-tr-sm px-4 py-2.5 text-sm leading-relaxed">{message.content}</div>
        </div>
      </div>
    )
  }
    ...
    <div className="flex items-start gap-2.5 px-4 py-1">
      ...
      <div className="max-w-[75%]">
        <div className={`rounded-2xl rounded-tl-sm px-4 py-2.5 ${sideStyle.bg}`}>
          <p className={`text-xs font-semibold mb-1 ${sideStyle.text} opacity-70`}>{sideStyle.label}</p>
          <p className={`text-sm leading-relaxed ${sideStyle.text}`}>{message.content}</p>
        </div>
