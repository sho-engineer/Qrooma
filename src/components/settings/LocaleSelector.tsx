...
export function LocaleSelector({ current }: Props) {
  ...
  function handleChange(next: Locale) {
    ...
    setLocaleState(next); setSaveState('saving')
    startTransition(async () => {
      await setLocale(next); setSaveState('saved')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-xs font-semibold text-gray-600 uppercase tracking-wide">UI Language / UI言語</h4>
      ...
      <div className="flex gap-2">
        {(['ja', 'en'] as Locale[]).map((l) => (
          <button key={l} onClick={() => handleChange(l)} disabled={isPending}
            className={`px-4 py-2 text-sm rounded-lg border transition-colors disabled:opacity-50 ${
              locale === l ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300 hover:border-blue-400'
            }`}>
            {l === 'ja' ? '日本語' : 'English'}
          </button>
        ))}
