...
interface Props { initial: 2 | 3; onSave: (count: 2 | 3) => Promise<{ error?: string }> }
...
export function AgentCountSelector({ initial, onSave }: Props) {
  ...
  function handleChange(newCount: 2 | 3) {
    setCount(newCount); setSaveState('saving')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-xs font-semibold text-gray-600 uppercase tracking-wide">{t.agentCount}</h4>
      ...
      <div className="flex gap-2">
        {options.map(({ value, label, desc }) => (
          <button key={value} onClick={() => handleChange(value)} disabled={isPending}
            className={`flex-1 text-left px-3 py-2.5 border rounded-lg transition-colors disabled:opacity-50 ${
              count === value ? 'border-blue-400 bg-blue-50' : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
            }`}>
            <p className={`text-sm font-medium ${count === value ? 'text-blue-800' : 'text-gray-800'}`}>{label}</p>
            <p className={`text-xs mt-0.5 ${count === value ? 'text-blue-600' : 'text-gray-500'}`}>{desc}</p>
          </button>
        ))}
      </div>
    </div>
